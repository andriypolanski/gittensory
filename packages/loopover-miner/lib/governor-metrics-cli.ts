...
function remainingRatio(decision: { allowed: boolean; remaining: number; limit: number }): number {
  const headroom = decision.allowed ? decision.remaining + 1 : 0;
  return headroom / decision.limit;
}

function collectRateLimitRows(
  buckets: GovernorRateLimitState["buckets"],
  nowMs: number,
): Array<{ scope: string; actionClass: string; repoFullName: string; ratio: number }> {
  const rows: Array<{ scope: string; actionClass: string; repoFullName: string; ratio: number }> = [];
...
function collectCapUsageRows(capUsage: GovernorCapUsage): Array<{ dimension: string; ratio: number }> {
...
function withGovernorState<T>(
  options: { openGovernorState?: () => GovernorState },
